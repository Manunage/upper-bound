...
class PointNet2ClassificationSSG(pl.LightningModule):
    ...
    def prepare_data(self):
        ...
        good_indices = np.load('/home/mnagel/PycharmProjects/Pointnet2_PyTorch/goodIndices.npy')
        good_indices = np.random.choice(good_indices, 100000)
        split_index = int(round(0.85*len(good_indices)))
        train_indices = good_indices[:split_index]
        val_indices = good_indices[split_index:]

        train_dataset = NuScenesLoaderXYZ(num_points=self.hparams["num_points"], transforms=None)
        self.train_dset = Subset(train_dataset, train_indices)

        val_dataset = NuScenesLoaderXYZ(num_points=self.hparams["num_points"], transforms=None)
        self.val_dset = Subset(val_dataset, val_indices)
